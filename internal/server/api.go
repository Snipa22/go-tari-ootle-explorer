...
import (
	...
	"github.com/Snipa22/go-tari-ootle-explorer/docs"
...
// (already a transitive dependency of this module via pgx's own test deps - see
// DISPATCH_BRIEF_OPENAPI.md's "skip the JSON transcoding feature entirely if it
// would require a new dependency" instruction, which this satisfies since no new
// dependency is added). A transcoding failure (which would only happen if the
// embedded YAML were malformed - a build-time programming error, not a runtime
// condition) falls back to a 500 JSON error rather than serving broken output.
